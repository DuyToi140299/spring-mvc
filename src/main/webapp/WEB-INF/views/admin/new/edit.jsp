		...
		<div class="page-content">
			<div class="row">
				<div class="col-xs-12">
					...
					<form:form class="form-horizontal" role="form" id="formSubmit" modelAttribute="model"> <!-- model này controller đẩy sang, các cái giá trị path ở dưới lấy từ model  -->
						<div class="form-group">
							  <label for="categoryCode" class="col-sm-3 control-label no-padding-right">Thể loại:</label>
							  <div class="col-sm-9">
							  	 <form:select path="categoryCode" id="categoryCode">     <!--  path = name + value -->]
						...
						<div class="form-group">
								...
								<div class="col-sm-9">
									<form:input path="title" cssClass="col-xs-10 col-sm-5"/> <!-- path đồng nghĩa với name và value -->
								</div>
						</div>
						<div class="form-group">
...
<script>
	$('#btnAddOrUpdateNew').click(function (e) {
	    ...
	    $.each(formData, function (i, v) {     //đẩy dữ liệu từ form vào mảng data
            data[""+v.name+""] = v.value;
        });
	    var id = $('#newId').val();
	    if (id == "") {
	    	addNew(data);     // thêm mới với mảng dữ liệu, vì sử dung api để call(dữ liệu là json) nên phải dùng mảng 
	    } else {
	    	updateNew(data);
	    }
	});
	...
	function addNew(data) {
		$.ajax({
            ...
            data: JSON.stringify(data),       // hàm này chuyển từ object -> json
            dataType: 'json',
	...
	function updateNew(data) {
		$.ajax({
            url: '${newAPI}',
            type: 'PUT',
            contentType: 'application/json',        //chuyển data object về json
